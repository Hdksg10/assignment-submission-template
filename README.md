...
* 步骤一：环境准备
  ...
  1. 配置集群节点，安装必要的软件和依赖。
  2. 启动Hadoop HDFS，确保NameNode和DataNode正常运行。
  3. 启动Apache Spark集群，确保Master和Worker节点正常运行。
  4. 启动Ray集群，确保Head和Worker节点正常运行。

* 步骤二：数据准备
  1. 下载所需数据集，将数据上传至HDFS，确保Spark和Ray能够访问数据。

* 步骤三：实验执行
  1. 在Spark集群上运行预处理算子，重复10次，记录执行时间和资源使用情况。
  2. 在Ray集群上运行相同的预处理算子，重复10次，记录执行时间和资源使用情况。
